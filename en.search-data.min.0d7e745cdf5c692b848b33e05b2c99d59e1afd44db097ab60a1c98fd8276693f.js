...
두 문자열을 사전 순으로 비교합니다 `}),e.add({id:3,href:"/docs/java/mordern_java_in_action/part1/",title:"Part 1. 기초",section:"Mordern Java in Action",content:` 1 장 자바 8, 9, 10, 11 : 무슨 일이 일어나고 있는가? # 스트림 API
...
public static \u0026lt;T\u0026gt; List\u0026lt;T\u0026gt; filter(List\u0026lt;T\u0026gt; list, Predicate\u0026lt;T\u0026gt; p ) { List\u0026lt;T\u0026gt; result = new ArrayList\u0026lt;\u0026gt;; for(T e: list) { if(p.test(e)) { result.add(e); } } return result; } 3장 람다 표현식 # 익명 : 보통 메서드와 달리 이름이 없으므로 익명이라 표현한다. 함수 : 특정 클래스에 종속되지 않으므로 메서드 대신 함수라고 표현한다. 전달 : 람다 표현식을 메서드 인수로 전달하거나 변수로 저장할 수 있다. 간결성 : 익명 클래스처럼 많은 코드를 구현할 필요가 없다. Comparator\u0026lt;Apple\u0026gt; byWeight = new Comparator\u0026lt;Apple\u0026gt;() { public int compare(Apple a1, Apple a2) { return a1.getWeight().compareTo(a2.getWeight()); } } Compartor\u0026lt;Apple\u0026gt; byWeight = (Apple a1, Apple a2) -\u0026gt; a1.getWeight().compareTo(a2.getWeight()); 함수형 인터페이스라는 문맥에서 람다 표현식을 사용할수 있다.
함수형 인터페이스
...
Function\u0026lt;Integer, Integer\u0026gt; f = x -\u0026gt; x + 1; Function\u0026lt;Integer, Integer\u0026gt; g = x -\u0026gt; x * 5; Function\u0026lt;Integer, Integer\u0026gt; h = f.andThen(g); int result = h.appy(1); //4 반환 Function\u0026lt;Integer, Integer\u0026gt; f = x -\u0026gt; x + 1; Function\u0026lt;Integer, Integer\u0026gt; g = x -\u0026gt; x * 5; Function\u0026lt;Integer, Integer\u0026gt; h = f.compose(g); int result = h.appy(1); //3 반환 `}),e.add({id:4,href:"/docs/java/basic/javatype/",title:"Primitive type, Reference type",section:"Basic",content:` Primitive type, Reference type # 1. 기본형 (Primitive) # Primitive type은 논리형, 문자형, 정수형, 실수형으로 구분된다. blooean, byte, short, int, long , float, duble, char 8가지 이다. null 이 존재하지 않는다. waper class 가 존재한다. Stack 메모리에 저장된다. 컴파일 시점에 메모리의 크기를 초과하면 에러가 발생한다. (stack over flow) 2. 참조형 (Reference type) # 기본형 타입을 제외한 타입들이 모두 참조형 타입(Reference type)이다. java.lang.Object 상속 null 이 존재하며 어떤 객체 주소도 저장되어 있지 않음을 의미한다. 객체 주소값을 저장 heap 메모리에 저장한다. 자료형(data type) 과 타입(type) 의 차이 # type 이 더 포괄적인 의미이다. 기본형과 다르게 참조형은 객체의 주소값을 저장하므로 data가 아닌 type 이라는 용어를 사용한다. (자바의 정석 - p.28)
`}),e.add({id:5,href:"/docs/infra/scale_up_scale_out/",title:"Scale-up, Scale-out",section:"Infra",content:` Scale-up, Scale-out # 스케일업(Scale-up)은 단순히 말하자면 기존의 하드웨어를 보다 높은 사양으로 업그레이드하는 것을 말합니다.
...
`}),e.add({id:6,href:"/docs/java/mordern_java_in_action/part2/",title:"Part2. 함수형 데이터 처리",section:"Mordern Java in Action",content:` Part2. 함수형 데이터 처리 # 4장 스트림 소개 # 스트림이란 무엇인가 Stream 자바 8 API 새로 추가된 기능이다.
자바 7의 코드
...
`}),e.add({id:8,href:"/docs/java/mordern_java_in_action/part5/",title:"Part5. 개선된 자바 동시성",section:"Mordern Java in Action",content:` 개선된 자바 동시성 # 15장 CompletableFuture와 리액티브 프로그래밍 컨셉의 기초 # 동시성을 구현하는 자바의 지원의 진화
...
자원 낭비 감소 : 기존 스레드를 재사용하여 스레드를 다시 만드는 오버 헤드를 줄일 수 있습니다. 스레드의 중앙 집중식 관리. 스레드 풀은 다양한 매개 변수를 표시하여 스레드를 모니터링하고 최적화하는 데 도움이되는 기능을 제공합니다. 15.3 박스와 채널 모델
`}),e.add({id:9,href:"/docs/java/mordern_java_in_action/part6/",title:"Part5. 함수형 프로그래밍과 자바 진화의 미래",section:"Mordern Java in Action",content:` 함수형 프로그래밍과 자바 진화의 미래 # 18장 함수형 관점으로 생각하기 # 18.1 시스템 구현과 유지보수
18.1.1 공유된 가변 데이터
...
함수를 이용하는 프로그래밍으로 함수의 의미는 수학적인 함수와 같다. 0개 이상의 인수를 가지며 한개 이상의 결과를 반환하지만 부작용(side-effect)이 없어야 한다. 시스템의 다른 부분의 영향을 미치지 않는다면 내부적으로는 함수형이 아닌 기능을 사용하는 방식을 함수형 프로그래밍이라 한다.
18.2.1 함수형 자바
...
공유 가변 자료구조를 줄이는 것은 장기적으로 프로그램 유지보수하고 디버깅하는 데 도움이 된다. 함수형 프로그래밍은 부작용이 없는 메서드와 선언형 프로그래밍 방식을 지향한다. 함수형 메서드는 입력 인수와 출력 결과만을 갖는다. 같은 인수 값으로 함수를 호출했을 때 항상 같은 값을 반환하면 참조 투명성을 갖는 함수다. while 루프 같은 반복움능 재귀로 대체할 수 있다. 자바에서는 고전 방식의 재귀보다 꼬리 재귀를 사용해야 추가적인 컴파일러 최적화를 기대할 수 있다. 19장 함수형 프로그래밍 기법 # 19.1 함수는 모든 곳에 존재한다. 자바 8부터는 함수도 일급으로 참조할 수 있다.
19.1.1 고차원 함수
...
`}),e.add({id:10,href:"/docs/database/mysql_index/",title:"Mysql Index",section:"Database",content:` MySQL 인덱스 # 인덱스(영어: index)는 데이터베이스 분야에 있어서 테이블에 대한 동작의 속도를 높여주는 자료 구조를 일컫는다. Multi Coulumn Index 는 인덱스 컬럼을 연결(concat)하여 정렬한 배열입니다.
...
SELECT * FROM test WHERE last_name=\u0026#39;Jones\u0026#39;; SELECT * FROM test WHERE last_name=\u0026#39;Jones\u0026#39; AND first_name=\u0026#39;John\u0026#39;; SELECT * FROM test WHERE last_name=\u0026#39;Jones\u0026#39; AND (first_name=\u0026#39;John\u0026#39; OR first_name=\u0026#39;Jon\u0026#39;); SELECT * FROM test WHERE last_name=\u0026#39;Jones\u0026#39; AND first_name \u0026gt;=\u0026#39;M\u0026#39; AND first_name \u0026lt; \u0026#39;N\u0026#39;; 위와 같이 접두사로 활용된 last_name 이 포함되여야 멀티 컬럼 인덱스가 사용되여 조회됩니다.
SELECT * FROM test WHERE first_name=\u0026#39;John\u0026#39;; SELECT * FROM test WHERE last_name=\u0026#39;Jones\u0026#39; OR first_name=\u0026#39;John\u0026#39;; 위와 같이 조회하면 멀티 컬럼인덱스가 사용되지 않습니다.
SELECT * FROM tbl_name WHERE col1=val1 AND col2=val2; col1 와 col2가 각각 컬럼 인덱스로 선언되어 있다면 옵티마이 저는 인덱스 병합 최적화를 사용하려고합니다.
참고 https://dev.mysql.com/doc/refman/8.0/en/column-indexes.html https://dev.mysql.com/doc/refman/8.0/en/multiple-column-indexes.html
`}),e.add({id:11,href:"/docs/database/mysql_trainsaction_level/",title:"Mysql Trainsaction Level",section:"Database",content:` MySQL Transaction Isolation Levels # MySQL의 Transaction Isolation Levels READ UNCOMMITTED
...
다른 트랜잭션에서 COMMIT 된 데이터만 읽어올 수 있는 level 한 트랜잭션에서 동일한 SELECT 쿼리의 결과가 다름(non-repeatable read) 이전의 SELECT 쿼리의 결과에 없던 row가 생김(phantom read) REPEATABLE READ
MySQL InnoDB의 기본 isolation level SERIALIZABLE
SERIALIZABLE 는 동시성을 상당 부분 포기하고 안정성에 큰 비중을 둔 isolation level 한 트MySQL의 Transaction Isolation Levels랜잭션에서 SELECT 쿼리를 실행하면 그 트랜잭션이 COMMIT 되기 전까지 다른 트랜잭션에서는 수정, 추가, 삭제 등의 작업조차 할 수 없으므로 아래 3가지 현상 모두 발생할 일이 없다. 참고 https://jupiny.com/2018/11/30/mysql-transaction-isolation-levels/#readcommitted
`}),e.add({id:12,href:"/docs/java/basic/java_charter/",title:"Java Charter",section:"Basic",content:" Java Character Converter # char to int # char c = \u0026#39;1\u0026#39;; int num = Character.getNumbericValue(c); int to char # int a = 65; char c = (char)a; char to String # char c = \u0026#34;C\u0026#34;; String s = String.ValueOf(c); String to char # String s = \u0026#34;hello\u0026#34;; char c = s.charAt(0); //returns h "}),e.add({id:13,href:"/docs/java/basic/java_map_sort/",title:"Java Map Sort",section:"Basic",content:` Java Map Sort by key and value # Map to List 를 활용 # import java.util.ArrayList; import java.util.Comparator; import java.util.HashMap; import java.util.List; import java.util.Map; import java.util.Map.Entry; import java.util.stream.Collectors; String[] array = new String[]{\u0026#34;sun\u0026#34;, \u0026#34;bed\u0026#34;, \u0026#34;car\u0026#34;}; Map\u0026lt;Integer, String\u0026gt; map = new HashMap\u0026lt;\u0026gt;(); map.put(2, array[2]); map.put(0, array[0]); map.put(1, array[1]); // key sort List\u0026lt;Integer\u0026gt; keyList = new ArrayList\u0026lt;\u0026gt;(map.keySet()); keyList.sort(Comparator.naturalOrder()); // 오름 차순 for (Integer key : keyList) { System.out.println(\u0026#34;Key: \u0026#34; + key); } keyList.sort(Comparator.naturalOrder()); // 내림 차순 for (Integer key : keyList) { System.out.println(\u0026#34;Key: \u0026#34; + key); } // entry set sort by value List\u0026lt;Entry\u0026lt;Integer,String\u0026gt;\u0026gt; entryList = new ArrayList\u0026lt;\u0026gt;(map.entrySet()); entryList.sort(Entry.comparingByValue()); entryList.forEach(System.out::println); // stream entry set sort by key List\u0026lt;Map.Entry\u0026lt;Integer, String\u0026gt;\u0026gt; list = map.entrySet().stream() .sorted(Map.Entry.comparingByKey()) .collect(Collectors.toList()); list.forEach(System.out::println); 참고
https://www.delftstack.com/ko/howto/java/how-to-sort-a-map-by-value-in-java/
`}),e.add({id:14,href:"/docs/java/basic/java_math/",title:"Java Math",section:"Basic",content:` Java Math # 제곱 구하기 static double	pow​(double a, double b)
double pow = Math.pow(2,2); 큰 수, 작은 수 static int min(int a , int b)
static double min(double a , double b)
static float min(float a , float b)
static long min(long a , long b)
int max = Math.max(2,3); int min = Math.min(2,3); `}),e.add({id:15,href:"/docs/spring/life_cycle/",title:"Life Cycle",section:"Spring",content:` Spring Bean Lifecycle # Lifecycle을 이용해서
...
Bean 생성 Bean의 생성은 IoC Container를 통해 진행(싱글톤 방식) 싱글톤 방식 -\u0026gt; 인스턴스가 없다 : 생성 / 있다 : 사용 방식으로 처리
Bean 초기화 Bean 생성 이후 초기화를 수행 초기화를 하기 위한 다양한 방법을 제공하고 있음 (토비 스프링 3.1 186P 참고) InitialIzingBean 구현 init-method지정 (xml or java annotation config) @PostConstruct 지정
Bean 사용 IoC Container를 통해 생성된 Bean을 마음껏 사용 반복적으로 사용되더라도 인스턴스가 신규로 생성되지 않는 한 초기화는 발생하지 않는다.
Bean 소멸 컨테이너가 종료될 때 destory 메소드가 선언되어 있다면 수행 소멸 시 사용할 수 있는 다양한 destroy 방법이 있음 DisponsableBean 인터페이스를 직접 구현 (destroy() 메소드) destroy-method를 넣어서 destory 시점에 동작 수행 (xml or java annotation config) @Predestory 지정
참고 자료
https://www.slipp.net/wiki/pages/viewpage.action?pageId=25528047
`}),e.add({id:16,href:"/docs/java/basic/javax_valid/",title:"javax @Valid",section:"Basic",content:` javax @Valid # implementation \u0026#39;org.springframework.boot:spring-boot-starter-validation\u0026#39; @NotNull : The annotated element must not be null. Accepts any type.
